...
class StomataGUI:
    def __init__(self, root):
        ...
        for prop_file in ["recent_BASE", "recent_ANNOT", "recent_CSV"]:
            ...
            # If a path isn't available, the user is prompted to select a new file to load.
                ...
                messagebox.showwarning(f"Missing File: {prop_file}")
                file_set = filedialog.askopenfilename(title=f"Select File: {prop_file}")
                self.config_properties[prop_file] = file_set
                self.config_properties[file_to_dir[prop_file]] = os.path.dirname(
                    file_set
                )
        ...
        for use_file in ["model_file"]:
            ...
            # If a path isn't available, the user is prompted to select a new file to load.
                ...
                self.config_properties[file_to_dir[use_file]] = os.path.dirname(
                    file_set
                )

        for prop_dir in ["dir_BASE", "dir_ANNOT", "dir_CSV"]:
        ...
        for use_dir in ["model_dir"]:
            try:
                assert os.path.isdir(self.config_properties[use_dir])
            except Exception:
                messagebox.showwarning(f"Missing Directory: {use_dir}")
                filedialog.askdirectory(title=f"Select Directory: {use_dir}")
        ...
        self.training_tab = ttk.Frame(self.main_tab_control)
        ...
        self.main_tab_control.add(self.training_tab, text="Training")
        ...
        self.file_label = tk.Label(
            self.image_compare_tab,
            text=os.path.basename(self.config_properties["recent_BASE"]),
        )
        self.file_label.grid(row=0, column=1, padx=10, pady=8)

        # Set up canvases
        self.canvas_base = tk.Canvas(
            self.image_compare_tab,
            width=self.window_sidelength,
            height=self.window_sidelength,
            bg="gray",
        )
        self.canvas_base.grid(row=1, column=0, padx=10, pady=8)

        self.canvas_annot = tk.Canvas(
            self.image_compare_tab,
            width=self.window_sidelength,
            height=self.window_sidelength,
            bg="gray",
        ...
        # Set up Buttons
        self.button_import_base = tk.Button(
            self.image_compare_tab,
            text="Import Base Image",
            command=self.import_BASE_dialog,
        )
        self.button_import_base.grid(row=2, column=0, padx=10, pady=10)

        self.button_import_annot = tk.Button(
            self.image_compare_tab,
            text="Import Annotation",
            command=self.import_ANNOT_dialog,
        )
        self.button_import_annot.grid(row=2, column=1, padx=10, pady=10)

        self.button_import_csv = tk.Button(
            self.image_compare_tab, text="Import CSV", command=self.import_CSV_dialog
        )
        ...
        # Clump ID Entry
        self.bbox_entry = tk.Entry(
            self.image_compare_tab, textvariable=self.bbox_number, width=4
        )
        ...
        # +/- (>>/<<) buttons
        self.button_decrement = tk.Button(
            self.image_compare_tab, text="<<", command=self.decrement_bbox
        )
        self.root.bind("q", self.decrement_bbox)
        self.button_decrement.grid(row=3, column=1, padx=5, pady=10)

        self.button_increment = tk.Button(
            self.image_compare_tab, text=">>", command=self.increment_bbox
        )
        ...
        # Advance checkbox
        self.checkbox_advance = tk.Checkbutton(
            self.image_compare_tab,
            text="Advance on Label",
            variable=self.advance_on_label,
            command=lambda: print(self.advance_on_label.get()),
        )
        ...
        if (
            "recent_BASE" in self.config_properties
            and self.config_properties["recent_BASE"]
        ):
            self.import_BASE(self.config_properties["recent_BASE"])
        if (
            "recent_ANNOT" in self.config_properties
            and self.config_properties["recent_ANNOT"]
        ):
            self.import_ANNOT(self.config_properties["recent_ANNOT"])
        if (
            "recent_CSV" in self.config_properties
            and self.config_properties["recent_CSV"]
        ):
    ...
    def import_BASE_dialog(self):
        ...
        file_path = filedialog.askopenfilename(
            ...
            initialdir=self.config_properties["dir_BASE"],
        )
        if file_path:
    ...
    def import_ANNOT_dialog(self):
        ...
        file_path = filedialog.askopenfilename(
            filetypes=[("Image files", "*.jpg *.jpeg *.png *.tif")],
            initialdir=self.config_properties["dir_ANNOT"],
        )
        if file_path:
    ...
    def import_CSV_dialog(self):
        ...
        decision = messagebox.askyesnocancel(
            "Get Annotation File", "Use premade clumps file?"
        )
        ...
        # No Path
            ...
            check = messagebox.askyesnocancel(
                "Confirm Annotation",
                "Do you have the correct file to make a clumps list of?",
            )
            # Yes Path
            outdir_newclumps = filedialog.askdirectory(
            ...
            if check:
                ...
                file_path = (
                    f"{outdir_newclumps}/"
                    + os.path.splitext(
                        os.path.basename(self.config_properties["recent_ANNOT"])
                    )[0]
                    + ".csv"
                )
                print(file_path)
    ...
    def update_overlay(self):
        if self.image_base and self.image_annot:
            try:
                assert self.image_base.size == self.image_annot.size, (
                    f"There's an image shape mismatch! Base: {self.image_base.size} || Annot: {self.image_annot.size}"
                )
                result = Image.blend(
                    self.image_base.convert("L"),
                    self.image_annot.convert("L"),
                    alpha=0.37,
                )
    ...
    def update_images_by_bbox(self):
        if (
            hasattr(self, "df_coords")
            and hasattr(self, "bbox_number")
            and self.bbox_number.get() < len(self.df_coords) + 1
            and self.bbox_number.get() >= 0
        ):
            self.update_bbox_coords(self.df_coords)
            self.update_image(self.canvas_base, self.image_base, "BASE")
            self.update_image(self.canvas_annot, self.image_annot, "ANNOT")
    ...
    def set_recents(self):
        with open(self.configFilePath, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            assert self.config_properties, (
                "This is an empty dict. I'm not writing that."
            )
            for key, value in self.config_properties.items():
    ...
    def set_test_split(self):
        file_path = filedialog.askopenfilename(
            filetypes=[("CSV Files", "*.csv")],
            initialdir=self.config_properties["dir_test_split"],
        )
        if file_path:
            return file_path
    ...
    def set_val_split(self):
        file_path = filedialog.askopenfilename(
            filetypes=[("CSV Files", "*.csv")],
            initialdir=self.config_properties["dir_val_split"],
        )
        if file_path:
            return file_path
    ...
    def run_inference(self):
        ...
        inf_path = filedialog.askopenfilename(
            filetypes=[("Image files", "*.jpg *.jpeg *.png *.tif")],
            initialdir=self.config_properties["dir_BASE"],
        )
        try:
        ...
        subprocess.run(
            f'python inference.py --model={pth_path} --input="{inf_path}" --overlap=128 --outputname="recent_GUI_output"',
            shell=True,
        )
        pass
