...
def quickerator(
    ...
    o: int,
    out: str,
    model: str = "checkpoints/comp_nosmalls_2025-05-21_12h-13m-25s/last.e029.pth",
) -> None:
    for file in glob.glob(dir + "/*.tif"):
        # name = os.path.basename(file)
        subprocess.run(
            f'python inference_SUF.py --model={model} --input="{file}" --overlap={o} --outputdir="{out}" --outputname="_o{o}" --progress=True --skip_empty=True',
            shell=True,
        )
...
if False:
    with open(
        "C:/Users/Gabriel/Documents/GitHub/PDA-Acquisition/install_standalone/nosmallsplitval_05-2025.csv",
        mode="r",
        newline="",
    ) as file:
        ...
        for row in reader:
            shutil.copy(
                row[0],
                "C:/Users/Gabriel/Documents/GitHub/PDA-Acquisition/publication_compare/inference/validation_set/BASE",
            )
            shutil.copy(
                row[1],
                "C:/Users/Gabriel/Documents/GitHub/PDA-Acquisition/publication_compare/inference/validation_set/ANNOT",
            )
if True:
    with open(
        "C:/Users/Gabriel/Documents/GitHub/PDA-Acquisition/install_standalone/nosmallsplitval_05-2025.csv",
        ...
        for row in reader:
            shutil.copy(
                row[0],
                "C:/Users/Gabriel/Downloads/samples_process_07302025/FORMER_COMPARE",
            )
        # subprocess.run(f"python contingency.py --ground_truth={file} --guess_image={f'../publication_compare/inference/inf_AZD/raw_inf_216/{inf}'} --texttag=FFcheck --mode=full_filter", shell=True)
