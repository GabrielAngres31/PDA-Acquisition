...
def run_training_mbn(
  ...
  trainingfolders: src.data.Folders,
  epochs: int,
  learning_rate: float,
  batchsize: int,
  pos_weight: float,
  checkpointdir: str,
  table_out: str = None,
  validationfolders: src.data.Folders | None = None,
):
  ...
  dataset = src.data.Dataset_mbn(trainingfolders) # DOES THIS WORK?
  loader = src.data.create_dataloader(dataset, batchsize, shuffle=True)
  vloader = None
  if validationfolders is not None:
    vdataset = src.data.Dataset_mbn(validationfolders)
    vloader = src.data.create_dataloader(vdataset, batchsize, shuffle=False)

    # TODO: IS THE DATA IN THE CORRECT FORMAT TO PASS TO A TRAINER?
