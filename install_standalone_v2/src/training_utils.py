...
def run_training(
    ...
    checkpointdir = os.path.join(
        checkpointdir, time.strftime(f"%Y-%m-%d_%Hh-%Mm-%Ss")
    )  # f"{model_ID}_" + time.strftime(f'%Y-%m-%d_%Hh-%Mm-%Ss'))
    ...
    if table_out:
        ...
        with open(
            f"{checkpointdir}{table_out}_all.csv", "w", newline=""
        ) as csv_all_out:
            csv_out_all_writer = csv.writer(
                csv_all_out, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
            [csv_out_all_writer.writerow([i]) for i in all_loss_list]
        if vloader:
            with open(
                f"{checkpointdir}{table_out}_V.csv", "w", newline=""
            ) as csv_out_v:
                csv_out_writer_v = csv.writer(
                    csv_out_v, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
                )
                [csv_out_writer_v.writerow([i]) for i in mean_vloss_list]
            with open(
                f"{checkpointdir}{table_out}_all_V.csv", "w", newline=""
            ) as csv_all_out_v:
                csv_out_all_writer_v = csv.writer(
                    csv_all_out_v,
                    delimiter=",",
                    quotechar='"',
...
def train_one_epoch(
    ...
    def distance_weights(
        image_size: tuple, center_weight: float = 5.0, edge_weight: float = 1.0
    ):
...
def run_training_mbn(
    ...
    dataset = torchvision.datasets.ImageFolder(
        training_folders, transform=src.data.to_tensor
    )
    loader = src.data.create_dataloader_mbn(dataset, batchsize, shuffle=True)
    vloader = None
    if validation_files is not None:
        vdataset = torchvision.datasets.ImageFolder(
            validation_files, transform=src.data.to_tensor
        )
    ...
    for e in range(epochs):
        ...
        try:
            ...
            try:
                ...
                print("Couldn't get LOSS_INFO!")
            try:
                print(f"[_mean]: {loss_info['mean']}")
            except:
                print("Couldn't get [_mean]!")
            try:
                print(f"[_losses]: {loss_info['losses']}")
            except:
                print("Couldn't get [_losses]!")
        scheduler.step()
        if vloader is not None:
        ...
        if table_out:
            ...
            with open(
                f"{checkpointdir}{table_out}_cloud.csv", "w", newline=""
            ) as csv_points_out:
                csv_points_writer = csv.writer(
                    csv_points_out,
                    delimiter=",",
                    quotechar='"',
                    quoting=csv.QUOTE_MINIMAL,
...
def train_one_epoch_mbn(
    ...
    losses = []
    assert str(loader)[1:39] == "torch.utils.data.dataloader.DataLoader", (
        f"{str(loader)[1:39]} is not 'torch.utils.data.dataloader.DataLoader'"
    )
    for i, [x, l] in enumerate(loader):
        x = augment_mbn(x)
        optimizer.zero_grad()
        y = module(x)
        loss = torch.nn.functional.cross_entropy(y, l)
        loss.backward()
        optimizer.step()
