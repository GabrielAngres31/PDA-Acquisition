...
def save_image(path: str, imagedata_f32: np.ndarray) -> None:
    assert imagedata_f32.dtype == np.float32, f"{imagedata_f32.dtype}"
...
def save_image_RGB(path: str, imagedata_u8: np.ndarray) -> None:
    assert imagedata_u8.dtype == np.uint8, f"{imagedata_u8.dtype}"
...
def cache_file_pairs(
    ...
    for inputpath, annotationpath in pairs:
        ...
        annotationpatches = slice_into_patches_with_overlap(
            annotationdata, patchsize, overlap
        )
...
def create_dataloader_mbn(
    ...
    **kw,
    ...
    print("running this - DEBUG (dataloader_mbn)")
    if num_workers == "auto":
        num_workers = os.cpu_count() or 1
    return torch.utils.data.DataLoader(
        ds,
...
def create_dataloader_fill_in(
    ...
    **kw,
    ...
    print("running this - DEBUG (dataloader_fill-in)")
    if num_workers == "auto":
        num_workers = os.cpu_count() or 1
    return torch.utils.data.DataLoader(
        ds,
...
def stitch_overlapping_patches(
    ...
    halfslack = overlap // 2
    i0, i1 = (
        grid[grid.shape[0] - 2, grid.shape[1] - 2, (2, 3)] - grid[-1, -1, (0, 1)]
    ) // 2
    d0 = np.stack(
        np.meshgrid(
    ...
    if out is None:
        out = torch.empty(
            patches[0].shape[:-2] + imageshape[-2:], dtype=patches[0].dtype
        )
    for patch, gi, di in zip(patches, d.reshape(-1, 4), (grid + d).reshape(-1, 4)):
        out[..., di[0] : di[2], di[1] : di[3]] = patch[
            ..., gi[0] : gi[2], gi[1] : gi[3]
        ]
    return out
