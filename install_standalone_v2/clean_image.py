...
from skimage.morphology import area_closing, area_opening, square
...
def main(args: argparse.Namespace) -> bool:
    ...
    clean_image(
        image_in, args.prediction_type, args.filter_type, args.save_image_as
    )  # Apply cleaning processes to image based on prediction type, filter option, and savepath.
...
def clean_image(
    ...
    if filter_mode == "otsu":
        ...
        if mode == "outlines":
            ...
            otsu_fill = area_closing(
                image > otsu_trsh_num, connectivity=square(3), area_threshold=2500
            )
            otsu_invt = invert(image > otsu_trsh_num)  # Invert the thresholded image

            inners = np.logical_and(
                otsu_fill, otsu_invt
            )  # Find the intersection of filled and inverted images
            otsu_clr = area_opening(
                inners, area_threshold=200
            ...
            print("Closing...")
            image_closing = area_closing(
                image_numpy, area_threshold=0
            )  # Perform area closing
            print("Opening...")
            image_opening = area_opening(
                image_closing, area_threshold=100
            )  # Perform area opening
            final_image = image_opening
    elif filter_mode == "confidence":
        # Apply confidence-based filtering
        final_image = (
            image / np.max(image) > 0.58
        )  # Normalize image and apply a confidence threshold
        final_image = area_closing(
            final_image, area_threshold=750
        )  # Perform area closing
        final_image = area_opening(
            final_image, area_threshold=350
        )  # Perform area opening
...
def get_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input_path", type=str, required=True, help="Image to find chunks for."
    )
    parser.add_argument(
        "--prediction_type",
        type=str,
