...
def main(args: argparse.Namespace) -> bool:
    ...
    assert os.path.exists(args.ground_truth), (
        f"Invalid path to ground truth image. Please check --ground_truth argument.\nArgument value: {args.ground_truth}"
    )
    assert os.path.exists(args.guess_image), (
        f"Invalid path to guess image. Please check --guess_image argument.\nArgument value: {args.guess_image}"
    )

    ground_truth_img = imread(
        args.ground_truth
    )  # Read in the ground truth image. Assumed to be black and white. HOWEVER, can be substituted with a greyscale image if comparing two inferences.
    guess_in_image = imread(
        args.guess_image
    )  # Read in the guess image. Assumed to be either black and white, or greyscale on [0,255]

    if guess_in_image.ndim == 3:
        guess_in_image = guess_in_image[
            :, :, 0
        ]  # Ensure that both images have only two dimensions, as RGB values should not be present in the image.
    ...
    if args.show_image:
        plt.show()
    plt.savefig(
        f"inference/contingencycompare/{os.path.basename(args.guess_image)}_{args.texttag}.png"
