...
def get_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--base_img", type=str, required=True, help="Path to base image."
    )
    parser.add_argument(
        "--compare_img", type=str, required=True, help="Path to comparison image."
    )
    parser.add_argument(
        "--threshold",
        type=int,
...
def main(args: argparse.Namespace) -> bool:
    ...
    if not args.output_path and not args.show_image:
        raise Exception(
            "You are neither saving nor displaying an image. Please provide a savepath to --output_path, or set --show_image to True."
        )
