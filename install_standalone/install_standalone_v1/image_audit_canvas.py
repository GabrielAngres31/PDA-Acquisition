...
class PixelCanvas:
    def __init__(
        ...
        for r, row in enumerate(self.annot_section_array):
            for c, col in enumerate(row):
                self.place_pixel(
                    None,
                    c * self.pixel_size + self.corr_margin * 2 + self.corr_width,
                    r * self.pixel_size + self.corr_margin,
                    self.annot_section_array[r, c],
                )
    ...
    def draw_initial_images(self):
        base_image = self.base_section.resize(
            (self.corr_width, self.corr_height), Image.Resampling.NEAREST
        )
        base_image_tk = ImageTk.PhotoImage(base_image)
        self.canvas.create_image(
            self.corr_margin, self.corr_margin, anchor=tk.NW, image=base_image_tk
        )
        self.canvas.image_base = base_image_tk

        over_image = self.annot_section.resize(
            (self.corr_width, self.corr_height), Image.Resampling.NEAREST
        )
        # over_image_tk = ImageTk.PhotoImage(over_image)
        over_assembly_image = Image.blend(
            base_image, over_image, self.overlay_alpha / 255
        )
        assemb_img_tk = ImageTk.PhotoImage(over_assembly_image)

        self.canvas.create_image(
            self.corr_margin * 3 + self.corr_width * 2,
            self.corr_margin,
            anchor=tk.NW,
            image=assemb_img_tk,
        )
        ...
        self.img_overlay_ann = self.base_section.resize(
            (self.corr_width, self.corr_height), Image.Resampling.NEAREST
        ).convert("RGBA")
        self.img_overlay_bas = self.base_section.resize(
            (self.corr_width, self.corr_height), Image.Resampling.NEAREST
        ).convert("RGBA")
    ...
    def check_canvas_coords(self, x, y):
        ...
        if main_x <= self.corr_margin:
            return False
        # if main_x <= self.corr_margin+self.corr_width:     return False
        if main_x >= self.corr_margin * 3 + self.corr_width * 3:
            return False
        if main_y <= self.corr_margin:
            return False
        if main_y >= self.corr_margin + self.corr_height:
            return False

        if (
            main_x >= self.corr_width + self.corr_margin
            and main_x <= self.corr_margin * 2 + self.corr_width
        ):
            return False
        if (
            main_x >= self.corr_width * 2 + self.corr_margin * 2
            and main_x <= self.corr_margin * 3 + self.corr_width * 2
        ):
            return False
        if (
            main_x >= self.corr_width * 3 + self.corr_margin * 3
            and main_x <= self.corr_margin * 4 + self.corr_width * 3
        ):
            return False
    ...
    def get_arraycoord_from_canvaspx(self, x, y):
        ...
        arr_coords = {
            "arr_x": main_coords["main_x"] - self.margin - compensate,
            "arr_y": main_coords["main_y"] - self.margin,
        }
        return arr_coords
    ...
    def place_pixel(self, canvas, x, y, fill=None):
        ...
        self.canvas.create_rectangle(
            x, y, x + self.pixel_size, y + self.pixel_size, fill="#" + fill_hex, width=0
        )
    ...
    def draw_pixel(self, x, y, draw_fill=None):
        ...
        # print(main_coords)
        if self.check_canvas_coords(
            x + 1, y + 1
        ):  # NOTE: I don't know why adding 1 to these fixes the off-by-one error.
            # Check here first if you get a weird lack of plotted pixels.
            self.place_pixel(
                None,
                main_coords["main_x"] * self.pixel_size,
                main_coords["main_y"] * self.pixel_size,
                draw_fill,
            )
        else:
            print(
                f"Couldn't draw here! You tried to draw at {main_coords['main_x']}, {main_coords['main_y']}"
            )
    ...
    def pixel_multitrack(self, event=None, x=None, y=None):
        ...
        ref_coord_annt = [
            (x % (self.corr_width + self.corr_margin))
            + self.corr_width
            + self.corr_margin,
            y,
        ]
        ...
        self.draw_pixel(
            ref_coord_ovly[0],
            ref_coord_ovly[1],
            draw_fill=(192 if self.drawcolor == 255 else 64),
        )
    ...
    def update_overlay(self, event=None):
        ...
        over_image = Image.fromarray(self.annot_section_array).resize(
            (self.corr_width, self.corr_height), Image.Resampling.NEAREST
        )
        # print(over_image)
        # over_image.show()
    ...
    def clear_annot(self):
        ...
        [
            self.place_pixel(
                None,
                coord=[
                    p % dim * self.pixel_size + self.corr_margin * 2 + self.corr_width,
                    p // dim * self.pixel_size + self.corr_margin,
                ],
                fill=0,
            )
            if flat_annot[p]
            else None
            for p in range(len(flat_annot))
        ]
    ...
    def set_all_white(self):
        ...
        self.annot_section_array[self.annot_section_array <= 40] = 0
    ...
    def eff_fill(self, event=None, x=None, y=None, fill=None):
        ...
        if self.annot_section_array[arr_y, arr_x] != fill:
            ...
            self.place_pixel(
                None,
                x * self.pixel_size + self.corr_margin * 2,
                y * self.pixel_size + self.corr_margin,
                fill=fill,
            )
            if y >= 0:
                self.eff_fill(None, x, y - 1)
            if x >= 0:
                self.eff_fill(None, x - 1, y)
            if x < self.annot_section_array.shape[1] - 1:
                self.eff_fill(None, x + 1, y)
            if y < self.annot_section_array.shape[0] - 1:
                self.eff_fill(None, x, y + 1)
        ...
        while points:
            ...
            if (x >= 0 and x < self.width) and (y >= 0 and y < self.height):
                ...
                if not self.annot_section_array[index[0], index[1]] == fill:
                    ...
                    self.draw_pixel(
                        x * self.pixel_size + self.corr_margin * 2 + self.corr_width,
                        y * self.pixel_size + self.corr_margin,
                        fill,
                    )
                    # print(arr_x)
    ...
    def custom_smoothing(self, event):
        ...
        for i in range(3):
            if np.any(
                (self.annot_section_array < 255) & (self.annot_section_array > 0)
            ):
            ...
            eraser = np.where(
                np.sum(
                    [shift_LEFT_brd, shift_RGHT_brd, shift_UP_brd, shift_DN_brd], axis=0
                )
                < 2
            )
            ...
            draw_delta_key = np.where(
                np.logical_xor(smoothed, self.annot_section_array)
            )
            draw_delta_coo = list(
                zip(draw_delta_key[0].tolist(), draw_delta_key[1].tolist())
            )
            ...
            for p in draw_delta_coo:
                # print(p[0])
                self.draw_pixel(
                    p[1] * self.pixel_size + self.corr_margin * 2 + self.corr_width,
                    p[0] * self.pixel_size + self.corr_margin,
                    draw_fill=self.annot_section_array[p],
                )
...
def main(args: argparse.Namespace) -> bool:
    ...
    canvas = PixelCanvas(
        ...
        64,
        64,
        base_section_path=args.base_path,
