...
def main(args: argparse.Namespace) -> bool:
    ...
    outf = os.path.join(
        args.outputdir, os.path.basename(args.input) + f"{args.outputname}.output.png"
    )
    # print(outf)
    # print("DIR_OUTPUT IS:")
...
def run_patchwise_inference(
    ...
    input_patches = src.data.slice_into_patches_with_overlap(
        x, patchsize=patchsize, overlap=overlap
    )
    ...
    if args.progress:
        iterator = tqdm.tqdm(input_patches)
    else:
...
def get_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--patchsize", type=int, default=256, help="Patch size, default 256"
    )
    ...
    parser.add_argument("--weights_only", default=True)
    parser.add_argument(
        "--skip_empty",
        default=False,
